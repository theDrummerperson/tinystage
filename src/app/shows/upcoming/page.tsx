    ...
    <main className='overflow-x-hidden bg-brand-black'>
      ...
      <section className='relative h-screen min-h-[700px] flex items-end overflow-hidden'>
        ...
        <div className='container mx-auto px-4 relative z-10 pb-20'>
          <div className='max-w-4xl'>
            ...
            <div
              className='flex flex-wrap gap-4 mt-8 motion-safe:animate-fadeIn'
            ...
            >
              ...
              >
                <Heart
                  className={`w-5 h-5 mr-2 transition-all ${isLiked ? 'fill-current scale-110' : 'group-hover:scale-110'}`}
                />
                {isLiked ? 'Liked' : 'Like'}
              </button>
            </div>
        ...
        <div className='container mx-auto px-4'>
          <div className='flex flex-wrap justify-center items-center gap-x-8 gap-y-2 text-sm text-brand-gray-light'>
            <div className='flex items-center'>
              ...
              <span>~60 min set</span>
            </div>
            <div className='flex items-center'>
              <Users className='w-4 h-4 mr-2 text-brand-yellow' />
              <span>Intimate Venue</span>
            </div>
            <div className='flex items-center'>
              <Music2 className='w-4 h-4 mr-2 text-brand-yellow' />
              <span>10 Songs</span>
            </div>
            <div className='flex items-center'>
              <Guitar className='w-4 h-4 mr-2 text-brand-yellow' />
          ...
          <div className='grid lg:grid-cols-3 gap-12 items-start'>
            <div className='lg:col-span-2 space-y-6'>
              ...
              <div className='prose prose-xl prose-invert max-w-none text-brand-gray-light leading-relaxed'>
                <p>
                  <span className='text-brand-yellow font-semibold'>Ellis</span>{' '}
                  (@_ellis_music) is singer-songwriter from Erie, PA, whose raw
                  honesty blurs the lines between alternative rock and emotional
                  storytelling.
                </p>
                <p>
                  Backed by guitarist{' '}
                  <span className='font-medium text-white'>Shawn Spencer</span>,
                  Ellis’s stripped-down live performances hit with emotional
                  clarity—letting the lyrics lead.
                </p>
              </div>
            </div>
            <div className='bg-brand-black/40 border border-brand-gray-dark/50 rounded-lg p-8 shadow-2xl'>
              ...
              <ul className='space-y-4 text-lg'>
                ...
                <li className='flex items-start'>
                  <Calendar className='w-6 h-6 mr-4 mt-1 text-brand-yellow/80' />
                  <span>
                ...
                <li className='flex items-start'>
                  <MapPin className='w-6 h-6 mr-4 mt-1 text-brand-yellow/80' />
                  <span>
                    <strong className='text-white block'>Venue:</strong>FEED
                    Media Downtown Arts Center
                  </span>
                </li>
                <li className='flex items-start'>
                  <MicVocal className='w-6 h-6 mr-4 mt-1 text-brand-yellow/80' />
                  <span>
                    <strong className='text-white block'>Genre:</strong>Acoustic
                    | Alt-Rock | Emo-Pop
                  </span>
                </li>
                <li className='flex items-start'>
                  <Guitar className='w-6 h-6 mr-4 mt-1 text-brand-yellow/80' />
                  <span>
                    <strong className='text-white block'>Accompaniment:</strong>
                    Shawn Spencer (Guitar)
                  </span>
                </li>
              </ul>
            </div>
          ...
          <div className='grid md:grid-cols-2 gap-4'>
            {setlist
              ...
              .map((song) => (
                <div
                  key={song.title}
                  className='group bg-brand-gray-dark/40 border border-white/10 rounded-lg p-5 hover:bg-brand-gray-dark/80 transition-colors'
                >
                  <div className='flex items-center justify-between'>
                    <div>
                      <h3 className='font-bold text-white text-lg group-hover:text-brand-yellow transition-colors'>
                        {song.title}
                      </h3>
                    ...
                    <div className='flex items-center space-x-3'>
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold ${song.type === 'Original' ? 'bg-brand-yellow/20 text-brand-yellow' : 'bg-brand-gray-light/20 text-brand-gray-light'}`}
                      >
                        {song.type}
                      </span>
                    </div>
          ...
          {setlist.length > 6 && (
            <div className='text-center mt-8'>
              <button
                ...
                className='px-8 py-3 border border-brand-yellow/50 text-brand-yellow rounded-full hover:bg-brand-yellow/10 transition-colors flex items-center mx-auto'
              >
                {showFullSetlist
                  ? 'Show Less'
                  : `Show All ${setlist.length} Songs`}
                <ChevronDown
                  className={`ml-2 w-5 h-5 transition-transform ${showFullSetlist ? 'rotate-180' : ''}`}
                />
              </button>
            </div>
          )}
      ...
      <section className='py-24 bg-brand-gray-dark'>
        <div className='container mx-auto px-4 text-center'>
          <h2 className='text-2xl md:text-3xl font-bold text-brand-white mb-8'>
            Explore More Live Music
          </h2>
          <div className='flex flex-wrap justify-center gap-4 md:gap-6'>
            <Link
              href='/shows'
              className='group inline-flex items-center justify-center px-8 py-4 border border-brand-yellow text-lg font-medium rounded-md text-brand-yellow hover:bg-brand-yellow hover:text-brand-black shadow-lg transition-all duration-300 ease-out transform hover:scale-[1.03]'
            >
            ...
            <Link
              href='/shows/archive'
              className='group inline-flex items-center justify-center px-8 py-4 border border-transparent text-lg font-medium rounded-md text-brand-black bg-brand-yellow hover:brightness-110 shadow-xl transition-all duration-300 ease-out transform hover:scale-[1.03]'
            >
