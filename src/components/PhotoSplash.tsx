...
const MobileCarouselView: React.FC<MobileCarouselViewProps> = React.memo(
  ({ images, currentIndex, onPrevious, onNext, onGoToSlide, onImageClick }) => {
    if (images.length === 0) {
      return (
        <p className='text-center text-brand-gray-light py-8'>
    ...
    return (
      <div className='relative w-full max-w-2xl mx-auto'>
        <div
          ...
          onClick={
            !currentImage.link ? () => onImageClick(currentIndex) : undefined
          }
          onKeyDown={
            !currentImage.link
              ? (e) =>
          ...
          aria-label={
            !currentImage.link
              ? `View image: ${currentImage.title || currentImage.alt} in lightbox`
              : undefined
          }
        >
          {currentImage.link ? (
                ...
                <button
                  ...
                  className={`w-2 h-2 md:w-2.5 md:h-2.5 rounded-full transition-all duration-300 ease-in-out ${
                    currentIndex === idx
                      ...
                      : 'bg-brand-gray-medium hover:bg-brand-gray-light scale-100'
                  }`}
...
const MainCtaButton: React.FC<MainCtaButtonProps> = React.memo(
  ({ text, link }) => (
...
export default function PhotoSplash({
  ...
  const currentLightboxPhoto = rawImagesData[lightboxImageIndex] as
    | LightboxPhoto
    | undefined;
    ...
    <section className='py-16 md:py-24 bg-brand-black text-brand-white font-sans overflow-hidden'>
      ...
      {isLightboxOpen && currentLightboxPhoto && (
        <LightboxView
          ...
          onPrevious={
            rawImagesData.length > 1 ? handleLightboxPrevious : undefined
          }
