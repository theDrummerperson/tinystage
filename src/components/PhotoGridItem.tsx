...
const PhotoGridItem: React.FC<PhotoGridItemProps> = React.memo(({ photo, isPriority, onImageClick }) => {
  const { src, alt, title, subtitle, link } = photo;
  // Subtle tilt, reduced scale slightly to feel more contained within the perspective
  const perspectiveHoverClasses = "group-hover:[transform:perspective(1000px)_rotateY(1deg)_rotateX(0.5deg)_scale(1.02)]";
    ...
    <div className="w-full h-full flex justify-center items-center p-0.5">
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-[rgba(var(--brand-black-rgb),0.85)] via-[rgba(var(--brand-black-rgb),0.5)] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out flex flex-col justify-end p-4 md:p-6 pointer-events-none">
        <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 ease-in-out pointer-events-auto">
          {title && <h3 className="text-xl font-semibold text-brand-white">{title}</h3>}
          {subtitle && <p className="mt-1 text-sm text-brand-gray-light">{subtitle}</p>}
        </div>
      </div>
    </div>
  );
  ...
  const commonProps = {
    ...
    onKeyDown: link ? undefined : ((e: React.KeyboardEvent) => (e.key === 'Enter' || e.key === ' ') && onImageClick()),
    "aria-label": link ? (title || alt) : `View image: ${title || alt} in lightbox`,
  };

  return link ? (
    <Link href={link} legacyBehavior>
      <a {...commonProps}> {/* Apply commonProps to the anchor tag */}
        {imageCardContent}
      </a>
    </Link>
  ) : (
    <div {...commonProps}> {/* Apply commonProps to the div */}
      {imageCardContent}
    </div>
  );
});
