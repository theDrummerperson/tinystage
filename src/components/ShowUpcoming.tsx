    ...
    <div className="min-h-screen bg-gradient-to-br from-brand-gray-dark via-brand-black to-brand-black font-sans text-brand-gray-light selection:bg-brand-yellow selection:text-brand-black flex flex-col items-center justify-center p-4 sm:p-6 md:p-8">
      <article className="bg-brand-black/30 backdrop-blur-xl border border-brand-gray-dark/50 rounded-2xl shadow-2xl w-full max-w-3xl lg:max-w-4xl overflow-hidden transform transition-all duration-500 hover:shadow-brand-yellow/20">
        <div className="md:flex">
          ...
          <div className="md:w-3/5 p-6 md:p-8 lg:p-10 flex flex-col space-y-5 md:space-y-6 overflow-y-auto max-h-[calc(100vh-4rem)] md:max-h-none"> {/* Scroll for smaller screens */}
            <header>
            ...
            <section className="space-y-3 text-sm md:text-base">
              <div>
                <h3 className="font-semibold text-brand-yellow/90 uppercase text-xs tracking-wider mb-1">Genre</h3>
                <p className="text-brand-gray-light">{show.genre.join(' | ')}</p>
              </div>
              <div>
                <h3 className="font-semibold text-brand-yellow/90 uppercase text-xs tracking-wider mb-1">Members</h3>
                <ul className="list-none text-brand-gray-light">
                  {show.members.map((member, i) => <li key={i}>{member}</li>)}
                </ul>
              </div>
              <div>
                <h3 className="font-semibold text-brand-yellow/90 uppercase text-xs tracking-wider mb-1">Hometown</h3>
                <p className="text-brand-gray-light">{show.hometown}</p>
              </div>
              <div>
                <h3 className="font-semibold text-brand-yellow/90 uppercase text-xs tracking-wider mb-1">Debut EP</h3>
                <p className="text-brand-gray-light">{show.debutEPStatus}</p>
              </div>
              <div>
                <h3 className="font-semibold text-brand-yellow/90 uppercase text-xs tracking-wider mb-1">TinyStage Setlist Preview</h3>
                <p className="text-brand-gray-light">{show.tinyStageSetlistPreview.join(' · ')}</p>
              </div>
            </section>
            ...
            <section className="prose prose-sm md:prose-base prose-invert max-w-none prose-p:my-2.5 prose-headings:text-brand-yellow/90 prose-strong:text-brand-white">
              ...
              <h3 className="font-semibold uppercase text-xs tracking-wider !mb-2 !mt-0">About {show.artistName}</h3>
              {bioParagraphs.map((paragraph, index) => (
                <p key={index} className="text-brand-gray-light/90 font-light leading-relaxed tracking-normal">
                  {paragraph}
                </p>
              ))}
            </section>
